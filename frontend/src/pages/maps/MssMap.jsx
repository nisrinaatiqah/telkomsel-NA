...
const MssMap = ({ element }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5001/api/sites/MSS`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans p-6 overflow-x-hidden">
      ...
      <div className="w-full h-[55vh] bg-white rounded-[3rem] shadow-2xl border-[12px] border-white overflow-hidden relative mb-10">
            ...
            <GeoJSON 
              ...
              onEachFeature={(f, layer) => {
                ...
                if (pData && pData.count > 0) {
                  ...
                  layer.on({ click: () => navigate(`/detail/MSS/${provName}`) });
                  layer.bindTooltip(`
                    <div class="p-4 font-black uppercase text-[10px] min-w-[200px] text-center">
                      ...
                      ${viewMode === 'occupancy' ? `
                        ...
                        <div class="text-slate-500 font-black">AVG LOAD: ${pData.avgOcc}%</div>
                      ` : `
                        <div class="flex flex-col items-center gap-2">
                          ...
                          <div class="text-[8px] text-gray-400 font-black">SUBSCRIBER USAGE</div>
                        </div>
                      `}
                    </div>
                  `, { sticky: true });
        ...
        <div className="absolute top-6 left-6 z-[1000] bg-white/90 backdrop-blur-md p-5 rounded-[2rem] shadow-xl border border-gray-100 min-w-[200px]">
          ...
          <div className="space-y-3">
            {viewMode === 'occupancy' ? (
              <>
                <div className="flex items-center gap-3"><div className="w-4 h-4 bg-[#ef4444] rounded-lg shadow-sm"></div><span className="text-[10px] font-black italic uppercase text-slate-700">&gt; 80% Critical</span></div>
                <div className="flex items-center gap-3"><div className="w-4 h-4 bg-[#ff7f00] rounded-lg shadow-sm"></div><span className="text-[10px] font-black italic uppercase text-slate-700">70-80% Warning</span></div>
                <div className="flex items-center gap-3"><div className="w-4 h-4 bg-[#eab308] rounded-lg shadow-sm"></div><span className="text-[10px] font-black italic uppercase text-slate-700">&lt; 70% Good</span></div>
              </>
            ) : (
              <>
      ...
      <div className="bg-white rounded-[3rem] shadow-xl p-10 border-4 border-white mb-20">
        <div className="flex flex-col md:flex-row md:items-center gap-5 mb-10 border-b border-gray-100 pb-8">
          ...
          <div className="md:ml-auto flex items-center gap-6 bg-slate-50 px-8 py-4 rounded-3xl border border-slate-100 font-black shadow-inner">
             <div className="text-right border-r pr-6 border-slate-200">
                <p className="text-[9px] text-slate-400 uppercase tracking-widest">Nationwide Nodes</p>
                <p className="text-3xl text-slate-800 italic leading-none">{sites.length}</p>
             </div>
             <div>
                {viewMode === 'occupancy' ? (
                  <>
                    <p className="text-[9px] text-red-400 uppercase tracking-widest">Global Load Avg</p>
                    <p className="text-3xl text-red-600 italic leading-none">
                      {(sites.reduce((acc, curr) => acc + cleanNum(curr.sub_occupancy), 0) / (sites.length || 1)).toFixed(2)}%
                    </p>
                  </>
                ) : (
                  <>
                    <p className="text-[9px] text-blue-400 uppercase tracking-widest">Total Usage (Nat)</p>
                    <p className="text-3xl text-blue-600 italic leading-none">
                      {sites.reduce((acc, curr) => acc + cleanNum(curr.sub_usage), 0).toLocaleString('id-ID')}
                    </p>
                  </>
                )}
             </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {Object.entries(statsByRegional).map(([name, data]) => (
            <div key={name} onClick={() => navigate(`/regional/MSS/${name}`)} className="bg-gray-50/50 p-6 rounded-[2.5rem] border-2 border-transparent hover:border-red-500 hover:bg-white transition-all group shadow-sm font-black uppercase italic cursor-pointer transform hover:scale-105">
      ...
      <BatchImportModal isOpen={isBatchModalOpen} onClose={() => setIsBatchModalOpen(false)} currentElement="MSS" onRefresh={fetchData} />
    </div>
  );
};
