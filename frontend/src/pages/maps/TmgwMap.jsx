...
const TmgwMap = ({ element }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Gagal Fetch Data TMGW:", err); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const processStats = (allSites) => {
    ...
    // Inisialisasi regStats agar selalu ada 11 regional
    ...
    allSites.forEach(site => {
      ...
      // 2. Stats per Regional (untuk Grid Bawah)
    ...
    Object.keys(provStats).forEach(k => {
      provStats[k].avgUtil = (provStats[k].totalUtil / provStats[k].count).toFixed(2);
    });

    // Hitung rata-rata regional
    Object.keys(regStats).forEach(k => {
      if (regStats[k].count > 0) {
        regStats[k].avgUtil = (regStats[k].totalUtil / regStats[k].count).toFixed(2);
      }
    });
  ...
  const getRegionStyle = (feature) => {
    ...
    let color = "#f1f5f9"; // Default jika kosong
    if (stat && stat.count > 0) {
      const util = parseFloat(stat.avgUtil);
      if (util >= 80) color = "#ef4444";      // Merah
      else if (util >= 50) color = "#eab308"; // Kuning
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans p-6 overflow-x-hidden">
      <div className="flex justify-between items-center mb-6">
        <button onClick={() => navigate('/')} className="flex items-center gap-3 font-black text-red-600 uppercase tracking-widest text-lg">
          <ArrowLeft size={28} strokeWidth={3} /> BACK
        </button>
        <div className="text-right">
          <h2 className="text-4xl font-black text-gray-900 uppercase italic tracking-tighter leading-none">TMGW <span className="text-red-600">HEATMAP</span></h2>
      ...
      <div className="w-full h-[55vh] bg-white rounded-[3rem] shadow-2xl border-[12px] border-white overflow-hidden relative mb-10">
          ...
          <MapContainer center={[-2.5, 118]} zoom={5} dragging={false} scrollWheelZoom={false} zoomControl={false} style={{ height: '100%', width: '100%' }}>
            ...
            <GeoJSON 
              ...
              onEachFeature={(f, layer) => {
                ...
                layer.on('click', () => navigate(`/detail/TMGW/${prov}`));
                layer.bindTooltip(`
        ...
        <button onClick={() => setIsBatchModalOpen(true)} className="absolute bottom-6 right-6 z-[1000] flex items-center gap-2 px-8 py-4 bg-red-600 text-white rounded-full text-xs font-black shadow-2xl hover:bg-black transition-all uppercase italic tracking-widest"><FileSpreadsheet size={18} /> IMPORT TMGW DATA</button>
      </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-xl p-10 border-4 border-white mb-20">
        <div className="flex items-center gap-5 mb-10 border-b border-gray-100 pb-8">
          ...
          <div><h3 className="text-3xl font-black text-gray-900 italic uppercase tracking-tighter leading-none">Regional Summary</h3><p className="text-[11px] text-gray-400 font-black uppercase tracking-widest mt-2">TMGW Trunk Performance Overview</p></div>
          <div className="ml-auto bg-slate-50 px-8 py-4 rounded-3xl border border-slate-100 flex items-center gap-6">
             <div className="text-right border-r pr-6 border-slate-200"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total TMGW</p><p className="text-3xl font-black text-slate-800 italic leading-none">{sites.length}</p></div>
             <div><p className="text-[9px] font-black text-red-400 uppercase tracking-widest">Avg National Util</p><p className="text-3xl font-black text-red-600 italic leading-none">
