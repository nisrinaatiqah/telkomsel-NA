...
const DetailTable = ({ isRegional = false }) => { 
  ...
  const fetchData = () => {
    ...
    const apiUrl = isRegional 
      ? `http://localhost:5001/api/sites/regional/${params.element}/${encodeURIComponent(locationName)}`
      : `http://localhost:5001/api/sites/${params.element}/${encodeURIComponent(locationName)}`;
    axios.get(apiUrl)
      .then(res => {
  ...
  // 1. Grouping Nama untuk DNS Gi (Contoh: BUARAN, KENANGA)
  const getNameGroups = () => {
  ...
  // 2. Hitung Status untuk DNS Gn / ADC
  ...
  // ==========================================
  // KONFIGURASI KOLOM (Sesuai Permintaan Excel)
  // ==========================================
  const columnConfig = {
    'DNS Gn': [
      { label: 'City', key: 'city' }, 
    ...
    'MSS': [
      { label: 'REGIONAL', key: 'regional_internal' }, // Sumbagut, dll
      { label: 'MSS ELEMENT', key: 'name' },           // MSMDN6, dll
      { label: 'VENDOR', key: 'vendor' },
      ...
      { label: 'CPU CATEGORY', key: 'cpu_category' }
    ],
    ...
    'GSS': [
      ...
      { label: 'CPU LOAD (%)', key: 'cpu_load' },
      { label: 'BHCA CATEGORY', key: 'bhca_category' },
      { label: 'CPU CATEGORY', key: 'cpu_category' }
    ],
  ...
  const currentCols = columnConfig[element] || [{ label: 'City', key: 'city' }, { label: 'Name', key: 'name' }];
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = sites.map((site, index) => {
      const rowData = [
        ...
        ...currentCols.map(col => {
          ...
          if (typeof val === 'number') val = val.toLocaleString('id-ID'); // Format koma Indonesia
          return `"${val}"`;
        })
      ];
      return rowData.join(",");
    ...
    link.href = url;
    link.download = `Export_${element}_${region}.csv`;
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pb-20">
      ...
      <div className="max-w-[98%] mx-auto p-8">
        ...
        <button 
          onClick={() => navigate(-1)} 
          className="flex items-center gap-2 mb-6 font-black text-red-600 uppercase tracking-widest text-sm hover:translate-x-[-5px] transition-all"
        >
        ...
        <div className="flex justify-between items-end mb-10">
          <div>
            ...
            <h2 className="text-3xl font-black text-red-600 uppercase tracking-tighter mt-2 leading-none">{locationName}</h2>
          </div>
          <button 
            onClick={handleExportCSV}
            className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-gray-200 rounded-2xl text-xs font-black shadow-sm uppercase tracking-wider hover:bg-gray-50 transition-all text-gray-700"
          >
          ...
          {element === 'DNS Gi' ? (
            ...
            <>
              ...
              {getNameGroups().slice(0, 3).map(([catName, count], i) => (
                <div key={i} className="bg-white p-6 rounded-[2.5rem] shadow-xl border-4 border-white flex items-center gap-5 transition-transform hover:scale-105">
                  <div className="p-4 bg-red-50 text-red-600 rounded-2xl"><Info size={28}/></div>
                  <div>
                    <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest leading-none mb-1">{catName}</p>
                    <p className="text-4xl font-black text-red-600 italic leading-none tracking-tighter">{count}</p>
                  </div>
                </div>
              ))}
            </>
          ) : (
            // Recap standar DNS Gn / ADC
            <>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-4 border-white flex items-center gap-5 transition-transform hover:scale-105">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-4 border-white flex items-center gap-5 transition-transform hover:scale-105">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest leading-none mb-1">Avg Cap</p>
                  <p className="text-4xl font-black text-gray-800 italic leading-none tracking-tighter">72%</p>
                </div>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-4 border-white flex items-center gap-5 transition-transform hover:scale-105">
                <div className="p-4 bg-slate-900 text-white rounded-2xl"><Database size={28}/></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[1600px]">
              ...
              <tbody className="divide-y divide-gray-100 font-bold text-[12px] text-gray-700">
                {loading ? (
                  <tr><td colSpan="40" className="p-24 text-center animate-pulse uppercase font-black text-gray-300 tracking-widest text-xl">Loading Infrastructure Data...</td></tr>
                ) : sites.length > 0 ? (
                  sites.map((site, index) => (
                    <tr key={site.id} className="hover:bg-blue-50/50 transition-colors group">
                      {/* Nomor Urut */}
                      <td className="px-6 py-6 text-center border-r border-gray-100 text-gray-400 font-medium bg-gray-50/50">
                        {index + 1}
                      </td>
                      ...
                      {currentCols.map(col => {
                        ...
                        // Jika value mengandung titik (desimal mentah), ganti jadi koma untuk tampilan
                        ...
                        // JIKA KOLOM ADALAH STATUS: Render Pill
                        if (col.key === 'status') {
                          const statusUpper = String(val).toUpperCase();
                          const isOnAir = statusUpper.includes('ON AIR') || statusUpper === 'ACTIVE';
                          return (
                            <td key={col.key} className="px-6 py-6 whitespace-nowrap border-r border-gray-50 last:border-0">
                              <span className={`px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${isOnAir ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-50 text-yellow-700 border border-yellow-100'}`}>
                                {val}
                              </span>
                            </td>
                          );
                        ...
                        // KOLOM DATA BIASA
                        return (
                          <td key={col.key} className="px-6 py-6 whitespace-nowrap border-r border-gray-50 last:border-0 group-hover:text-red-600 transition-colors">
                            {val}
                          </td>
                        );
                      })}
